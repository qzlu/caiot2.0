...
export default {
  ...
  computed: {
    ...
    linkFormList(){
      let fIndex = this.$route.params.id
      console.log(fIndex,this.formList.filter(item => item.FIndex != fIndex ))
      return this.formList.filter(item => item.FIndex != fIndex )
    }
  },
  watch:{
    $route(route){
  ...
  methods: {
    initSystem(){
      ...
      this.currentForm = this.formList.find(item => item.FIndex == formIndex)
